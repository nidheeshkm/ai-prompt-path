...
export default function ChapterPage() {
  ...
  const completedCount = chapter.topics.filter(t => progressMap[t.id]?.status === 'completed').length
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8">
        <div className="max-w-3xl mx-auto">
          <Link href="/dashboard" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-gray-200 mb-6">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl border border-gray-800/50 bg-gray-900/50 opacity-50 cursor-not-allowed">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs text-gray-600">Complete previous topic to unlock</span>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-gray-800">
            {chapterId > 1 && (
              <Link href={`/learn/${chapterId - 1}`} className="text-sm text-gray-400 hover:text-gray-200">
                &larr; Chapter {chapterId - 1}
              </Link>
            )}
            <div />
            {chapterId < curriculum.length && (
              <Link href={`/learn/${chapterId + 1}`} className="text-sm text-gray-400 hover:text-gray-200">
                Chapter {chapterId + 1} &rarr;
              </Link>
            )}
          </div>
        </div>
